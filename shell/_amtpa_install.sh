#!/bin/bash
lines=28
install_path="/usr/local/amtpa"
install_tar="amtpa_install.tar.gz"
tail -n +$lines $0 > $install_tar
tar jxvf $install_tar
mkdir $install_path
chmod 777 $install_path
cp -r install/amtpa_bin/* $install_path
rm -rf install/amtpa_bin
rm -f $install_tar
...
cp $install_path/libamtpca.so.1.0.0 /usr/lib64
ln -snf /usr/lib64/libamtpca.so.1.0.0  /usr/lib64/libamtpca.so
